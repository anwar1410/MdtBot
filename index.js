 ...
 if (interaction.type === InteractionType.ModalSubmit && interaction.customId === 'edit_identity_modal') {
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'my_identity_select') {
        ...
        if (interaction.values[0] === 'my_info') {
            ...
            // توليد صورة الهوية باستخدام canvas بدلاً من Jimp
            ...
            ctx.font = '16px Arial';
            ctx.fillStyle = '#333';
            ctx.fillText('*** : الرقم', 350, 100);

            // تاريخ الميلاد
            ctx.font = '16px Arial';
            ctx.fillText(`تاريخ الميلاد : ${identity.day.padStart(2, '0')}/${convertArabicMonthToNumber(identity.month)}/${identity.year}`, 350, 150);

            // المدينة
            ctx.font = '16px Arial';
            ctx.fillText(`المدينة : ${identity.city}`, 350, 200);
